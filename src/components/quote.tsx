...
export default function RQuote({
  ...
  const getColor = () => {
    return quoteState[1] === "public/irohmp3/iroh0.mp3"
      ? "#2E3532"
      : quoteState[1] === "irohmp3/iroh1.mp3"
      ? "#C7CEDB"
      : quoteState[1] === "irohmp3/iroh2.mp3"
      ? "#94849B"
      : quoteState[1] === "irohmp3/iroh3.mp3"
      ? "#A0AAB2"
      : quoteState[1] === "irohmp3/iroh4.mp3"
      ? "#d5ff80"
      : quoteState[1] === "irohmp3/iroh5.mp3"
      ? "#242F40"
      : quoteState[1] === "irohmp3/iroh6.mp3"
      ? "#F9E784"
      : quoteState[1] === "irohmp3/iroh7.mp3"
      ? "#685044"
      : quoteState[1] === "irohmp3/iroh8.mp3"
      ? "#8075FF"
      : quoteState[1] === "irohmp3/iroh9.mp3"
      ? "#E5D1D0"
      : quoteState[1] === "irohmp3/iroh10.mp3"
      ? "#ccccff"
      : "#2A2C24";
  };
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                className={`${
                  photomode ? "hidden" : ""
                } mainBtn share head absolute top-5`}
    ...
    return (
      <div>
        ...
        <Button
          ...
          className={`${photomode ? "hidden" : ""} mainBtn ml-3 share`}
          variant="outline"
          size="icon"
        >
  ...
  return (
    <div
      ...
      style={{ backgroundColor: bgColor, transition: "background-color 1s" }}
    >
      <Card
        className={`${
          outlineFade ? "blur-sm" : ""
        } quotecard p-3 max-w-lg mb-40 mx-6`}
      >
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  className={`${
                    photomode ? "hidden" : ""
                  } mainBtn share absolute right-6 top-5`}
                  variant={outlineFade ? "ghost" : "outline"}
                >
        ...
        <CardHeader>
          <CardDescription className="smol text-sm md:text-base lg:text-lg xl:text-lg pt-0.5">
            Uncle Iroh says...
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div
            className={`flex my-4 ${photomode ? "cursor-pointer" : ""}`}
            onClick={photomode ? switchMode : undefined}
          >
            <Quote className="flex-initial" style={{ flexBasis: "20%" }} />
            <p
              className={`p-1 ml-3 text-base md:text-base lg:text-lg xl:text-xl ${
        ...
        <CardFooter className="shadfooter text-sm md:text-base lg:text-lg xl:text-lg">
          ...
          <div
            className={`${
              photomode ? "hidden" : ""
            } relative ml-3 text-xs md:text-xs lg:text-sm xl:text-sm`}
          >
